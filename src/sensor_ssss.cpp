...
int  sensor_ssss_acquisition_buffer_ready()
{
    ...
    uint16_t pixels_temp[PIXEL_NUM] = {0};

    /*Read temp*/
    amg8833_sensor.read_pixel_temperature_reg_value(pixels_temp);

    /* Fill this accelerometer data into the current data block */
    int16_t *p_amg8833_data = (int16_t *)p_dest;

    for (int i = 0; i < PIXEL_NUM; i++) {
       *p_amg8833_data = pixels_temp[i];
       p_amg8833_data++;
    }
